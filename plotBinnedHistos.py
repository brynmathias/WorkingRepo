...
# here we plot either all the hists in teh file, or put a list in for the histos to plot.
histList = GetAllhistos(resultsDir[0]+"/Data/AK5Calo_Jets.root")#["HT_aftar_AlphaT_all"]#"JetMultiplicityAfterAlphaT_all",
JetThreshList = ["","37","43"]
for num in JetThreshList:
  for dir in GetAllSubFiles("../results/Data/AK5Calo_Jets.root"):
    ...
    for hist in histList:
      ...
      errorValDa = Root.Double(0)
      errorValMC = Root.Double(0)
      print "Data",dir,hist,num,Data.IntegralAndError(0,-1, errorValDa),"+/-",errorValDa
      print "MC",dir,hist,num, MCCentral.IntegralAndError(0,-1, errorValMC),"+/-",errorValMC
      c1.cd(1)
      Data.Draw("p")
