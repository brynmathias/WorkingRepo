...
void L1JetAnalysis_2011::BookHistos() {
  ...
  L1JetEt = new TH1F("L1MatchedJetEt","L1Et",1000,0.,1000.);
...
double L1JetAnalysis_2011::deltaPhi(double phi1, double phi2) {
  ...
  if (fabs(result)> 100.) return result;
...
// --------------------------------------------------------------------
  ...
  //loop over the events
    for (Long64_t i= 0; i<   nevents; i++)
    {
      ...
      //cout << "Number of Jets in event = " << recoJet_->et.size() << endl;
      // if(!PassTrig(1040,2)) continue;
      // for(unsigned int t = 0; t < event_->hlt.size(); ++t){
      //  cout << event_->hlt[t] << endl;
      // }
     // bool trigger = std::find(event_->hlt.begin(),event_->hlt.end(),"HLT_Mu20_v1")!=event_->hlt.end();
    // if(!trigger) continue;
     ...
     L1JetEt->Fill(ReturnMatchedQuantity(matchedJet,Et),wgt);
